...
{ 
  char INFITS[128],CONST[128], param[128];
  double Umax,Umin,kv,cutoff;
  int Nbin;
  FILE *fp;

  if(argc!=5){
    printf("Usage: %s <input FITS file> <Norm FITS file> <input file> <outputfile> \n", argv[0]);
      return 1;
  }
  
  sscanf(argv[1],"%s",INFITS);
  sscanf(argv[2],"%s",CONST);
  
  //reading input parameter
  fp=fopen(argv[3],"r");

  fgets(param,100, fp);
  sscanf(param,"%lf%lf",&Umin, &Umax);
  
  fgets(param,100, fp);
  sscanf(param,"%d%lf",&Nbin, &cutoff);
  ...
  double UvalGrid,*binval,*uval,*weight,*num,wg;
