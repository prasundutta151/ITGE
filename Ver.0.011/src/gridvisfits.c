...
{ 
  char INFITS[128],OUTFITS[128],input[128],OPT[1], param[128];
  ...
  fgets(param,100, fp);
  sscanf(param,"%d%d",&chan1, &chan2);
  
  fgets(param,100, fp);
  sscanf(param,"%lf%lf%lf",&Umax, &Umin, &dU);
  
  fgets(param,100, fp);
  sscanf(param,"%lf%d",&fac, &flagLL);
  ...
  printf("INFITS = %s Umax = %e Umin = %e Chan1 = %d Chan2 = %d\n", INFITS, Umax, Umin, chan1, chan2);
  ...
  for(ii=0; ii<n_ave; ii++)
    {    
      corrfact[ii] = 1.+ fac*(del_chan/nu_chan0)*(chan1+ii+0.5-chan0);//lamda[chan0]/lambda[ii]
      printf("corrfact[%d] = %e\n", ii, corrfact[ii]);
      corrfact[ii] = 1.;
    }
  corrfact[0] = 1.;
  ...
  for(ii=0;ii<nbasln;ii++)
      ...
      for(chan=0;chan<n_ave;++chan)
	{
	  ...
	  uval = uuc*uuc+vvc*vvc;
	  // printf("Uval = %e\tUmax = %e\n", sqrt(uval), Umax);
	  if(uval <= Umax*Umax)
	    {	  
  ...
  printf("NData = %e Mb\n", 1.*Ndata*sizeof(float)/1024./1024.); 
