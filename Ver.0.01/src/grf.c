...
int main(int argc, char *argv[])
{
  ...
  double p11=0.,p22=0.;
  char OPT[1],psnbeam[150],pswbeam[150], param[128];
  ...
  fgets(param,100, fp);
  sscanf(param,"%ld%d%lf",&seed, &N, &L);
  
  fgets(param,100, fp);
  sscanf(param,"%lf%lf%lf%d",&nu0, &deltanu, &chan0, &Nchan);
  
  fgets(param,100, fp);
  sscanf(param,"%d%lf",&NB, &theta0);
  
  fgets(param,100, fp);
  sscanf(param,"%lf%lf",&spindex, &delspindex);
  
  fgets(param,100, fp);
  sscanf(param,"%lf%lf",&A_150, &betav);
 
  fgets(param,100, fp);
  sscanf(param,"%s", psnbeam);

  fgets(param,100, fp);
  sscanf(param,"%s", pswbeam);
...
void SWRITE_HDR(char *outfile)
{
  fitsfile *fitsp;
  int ii, keyn;
  char *CTYPE[3] = {"thetax", "thetay", "CHAN"};
  char keynam[6];
